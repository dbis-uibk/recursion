			...
			for(i=0;i<g.length;i++){
				...
				q.WHERE=function(){
					...
					for(var i in ewhere){
						...
						//fix so that table.column also works with group by
						if(!this.hasOwnProperty(i) && i_alt.length > 1){
							i1 = i_alt[0];
							i2 = i_alt[1];
							
							x=x && this[i1][i2]==ewhere[i];
						}
				...
				for (j=0;j<s.length;j++){
					...
					if (typeof s[j-1]=="string"  && s[j-1].charAt(0)=="|" && s[j-1].charAt(s[j-1].length-1)=="|"){
						...
						if(ag_type == "calc"){
							...
							console.log("ATT\n"+JSON.stringify(att));
							for(var k in att){
								if(d[i].hasOwnProperty(att[k])){
									att[k] = d[i][att[k]];
								}
								
//								if(!isNaN(att[k]) || att[k] == '+' || att[k] == '-' || att[k] == '*' || att[k] == '/' || att[k] == '%'){
//									continue;
//								}
//								
//								if(att[k].indexOf('.') >= 0){
//									att[k] = parseFloat(att[k]);
//								}
//								else{
//									att[k] = parseInt(att[k]);
//								}
//								
//								if(isNaN(att[k])){
//									nan = true;
//								}
							...
							if(nan){
								...
								//console.log(eval(att.join("")));
								theVal = eval(att.join(""));
							}
							pname = pname;
						}
						else if(ag_type == "concat"){
							var att = s[j].slice();
							for(var k in att){
								if(d[i].hasOwnProperty(att[k])){
									att[k] = d[i][att[k]];

/*
20-02-2016
 * added basic with-syntax

17-02-2016
 * added aggregate functions to having

16-02-2016
 * fixed group by/having
 * added aggregate functions

09-02-2016
 * like/between
 * added joins
 * wrong indexing for table.column
*/
{
...
sql_stmt_list =
  r: ( whitespace ( with_clause )? whitespace semicolon )+ 
...
with_clause = 
	clause_def:  WITH whitespace1  cname: ( name ) whitespace lparen
    			whitespace clause_att: ( name ( comma whitespace name )* ) 
                rparen AS whitespace
	lparen whitespace query_def: select_stmt whitespace rparen
    outer_query: select_stmt
    { var res = {};
...
select_core =
  ( SELECT d: ( ( DISTINCT / ALL )? )
    ...
    w: ( e: ( ( WHERE expr )? )
         { return e ? e[1] : [] } )
    g: ( GROUP BY  ordering_term ( whitespace comma ordering_term )* ( HAVING expr )? )? )
  { c[1].unshift(c[0]);
    var res = { results: c[1] };
...
select_result =
  r: ( whitespace
       ( ( c: ( agg_column_ref ( a: ( AS whitespace column_alias )
                             { return { alias: a[2] } } )? )
...
agg_column_ref = 
	( a: ( agg_function whitespace lparen column_ref rparen )
    	{ return merge(a[3],{ agg: a[0] } ) } ) 
...
agg_function = 
	x: ( whitespace 
    		( 'COUNT'i / 'SUM'i / 'AVG'i / 'MIN'i / 'MAX'i ) )
	{ return x[1] }
...
COUNT = whitespace1 "COUNT"i
SUM = whitespace1 "SUM"i
MIN = whitespace1 "MIN"i
MAX = whitespace1 "MAX"i
AVG = whitespace1 "AVG"i
WITH = whitespace "WITH"i
